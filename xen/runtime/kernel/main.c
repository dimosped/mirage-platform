...
static s_time_t block_nsecs;

CAMLprim value
caml_block_domain(value v_timeout)
{
  CAMLparam1(v_timeout);
  // v_timeout is provided as a floating point value, indicating the timeout duration in seconds
  block_nsecs = (s_time_t)(Double_val(v_timeout) * 1000000000);
  set_xen_guest_handle(sched_poll.ports, ports);
  sched_poll.nr_ports = sizeof(ports) / sizeof(evtchn_port_t);
  sched_poll.timeout = NOW() + block_nsecs;
